...
extern SoftI2CMaster i2c;// = SoftI2CMaster( 5, 3 );
#include <limits.h>
...
{
  i2c.beginTransmission(L3GD20_ADDRESS);
  #if ARDUINO >= 100
...
byte Adafruit_L3GD20_Unified::read8(byte reg)
{
  ...
  i2c.beginTransmission((byte)L3GD20_ADDRESS);
  #if ARDUINO >= 100
    i2c.write((uint8_t)reg);
  #else
    i2c.send(reg);
  #endif
  i2c.endTransmission();
  i2c.requestFrom((byte)L3GD20_ADDRESS);
  //while (!i2c.available()); // Wait for data to arrive.
  #if ARDUINO >= 100
    value = i2c.read();
  #else
    value = i2c.receive();
...
bool Adafruit_L3GD20_Unified::getEvent(sensors_event_t* event)
{
  ...
  while(!readingValid)
  {
    ...
    i2c.beginTransmission((byte)L3GD20_ADDRESS);
    #if ARDUINO >= 100
      i2c.write(GYRO_REGISTER_OUT_X_L | 0x80);
    #else
      i2c.send(GYRO_REGISTER_OUT_X_L | 0x80);
    #endif
    ...
    #else
      uint8_t xlo = i2c.receive();
      uint8_t xhi = i2c.receive();
      uint8_t ylo = i2c.receive();
      uint8_t yhi = i2c.receive();
      uint8_t zlo = i2c.receive();
      uint8_t zhi = i2c.receive();
    #endif    
